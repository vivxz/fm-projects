...
let myAny: any = 32;
let myUnknown: unknown = "hello, unknown"; // unknown can receive any value but different from any –– access

// Note that we can do whatever we want with an any, but nothing with an unknown

myAny.foo.bar.baz;
// myUnknown.foo; // must narrow it before using
 ...
 */
async function logWhenResolved(p: Promise<any>) { // use any to retain flexibility
  const val = await p;
...
bb = aa; // 🚨 yikes –– any unknown could be reassigned to any other unknown without any error
...
if (typeof y === "string") {
  ...
  // y is a number here
  y.toFixed(2);
} else { // considered all posibilities, else there would be an error on y –– exhaustive conditional
