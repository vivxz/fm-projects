...
import * as ts from "typescript"; // wild card import to get everything from typescript
...
const nonDeclFiles = program 
  .getSourceFiles()
  .filter(sf => !sf.isDeclarationFile); // filtering out dts files

// (3) get the type-checker –– takes all types and interface and binds them together and see what is going to work
...
const sfSymbols = nonDeclFiles
  .map(f => checker.getSymbolAtLocation(f)) // location of file
...
sfSymbols.forEach(sfSymbol => {
  ...
  console.log(sfSymbol.name); // log out each symbols name
  if (fileExports) {
    ...
    fileExports.forEach((value, key) => {
      ...
      // getting it into a form that is readable
